...
export const gcondimB: Phrase = {
  ...
  // 度数の流れ：△3 → 5 → ♭7 → ♭9 → △3 → ♭9 → ♭7 → 5 →（Cm7のルート=C へ解決）。上行→折り返し→下行の対称形。
  licks: [
    {
      name: "コンディミ・アルペジオ",
      note: "△3(B)から上行アルペジオ B-D-F-Ab-B（頂点）→ 折り返して Ab-F-D と下行 → 4弦10f(C)でCm7のルートへ解決。骨格はG#dim(Ab B D F)",
      seq: [
        ...
        { s: 2, f: 10, deg: "♭7" }, // 3弦10fr = F（♭7）
        { s: 3, f: 12, deg: "P5" }, // 4弦12fr = D（5）
        { s: 3, f: 10, deg: "R", resolve: true }, // 4弦10fr = C = Cm7 のルートへ解決
      ],
    },
  ],

  point:
    "Gコンディミスケールの5弦ルートポジション（8〜12fr）。G7(b9)上でコードトーン（△3, 5, b7）とb9を使った上行アルペジオから折り返して下行する対称形フレーズ。G#dim（Ab B D F）の構成音がアルペジオの骨格となっている。最後はCm7のルート(C)へ解決。",
};
